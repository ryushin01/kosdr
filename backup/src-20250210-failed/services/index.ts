  ...
  function(error) {
    ...
    if (error.response && error.response.status) {
      switch (error.response.status) {
        ...
        // Error1 Acceptable: HTTP headers의 content-type이 부적절한 경우
        case 406:
          console.log("Error1 Acceptable: HTTP headers의 content-type이 부적절한 경우");
          return new Promise(() => {
          });
        ...
        // Internal Server Error1: 서버 오류
        case 500:
          console.log("Internal Server Error1: 서버 오류");
          return new Promise(() => {
          });
        default:
          return Promise.reject(error);
